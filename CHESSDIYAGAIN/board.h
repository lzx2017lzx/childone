...
struct Step
{
    bool selectRedMovestep;
    int selectIDstep;
    int selectstep;
    int killIDstep;
    MyCPieces mycparrstep[32];//每个棋子的col信息和row信息在行动中可能变化，必须保存
...
public:
    ...
    int selectID;//选中的棋子的ID。回退时，可以通过selectID确定哪一方回退。
    int killID;//记录杀掉的棋子ID。功能只有一个，那就是方便回退。
    ...
    QList<Step>steps;//定义每一步的所有棋盘信息的链表，只存储棋子移动后的棋盘所有重要信息

    void save(QString filename);
    void open(QString filename);
    void RegretRecord();//regretrecord函数，只在棋子移动后调用
    void Goback();//晦棋函数,要考虑当红旗移动后，红旗想晦棋，和当黑棋移动后，黑棋想晦棋两种情况，可以通过比较调用晦棋函数时selectRedMove与容器的back元素是不是同号，同号说明悔两步，异号说明悔一步
    void Gminitiate();//初始化游戏
signals:
