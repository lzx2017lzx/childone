...
{
public:
    static Config *getInstance()//1、定义公有静态成员函数，返回静态成员(一个指向对象的类指针)，供外界访问，实际上是向外界传递了地址,懒汉式的对象是在成员函数中申请对象
    {
        if(NULL==pConfig)
        {
            pConfig=new Config;
...
//懒汉式指针首先赋值NULL
Config *Config::pConfig=NULL;//因为static已经申明了，所以尽管pConfig是私有成员，任然不被视为类外。
