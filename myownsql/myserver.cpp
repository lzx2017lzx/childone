...
int main()
{
	...
	while (1)
	{
		...
		{	
			...
			const char *passwordtemp=MYMD5(password);
			printf("passwordtemp:%s\n",passwordtemp);
			//sprintf(sql_buf, "select * from tuser where username = '%s' and password = '%s';", username, passwordtemp);
			sprintf(sql_buf, "select * from tuser where username = '%s' and fpassword = '%s';", username, password);
			printf("sql_buf:%s\n",sql_buf);
